...
public class OwnerInfo {
    ...
    public OwnerInfo(Context content) {
        final AccountManager manager = AccountManager.get(content);
        final Account[] accounts = manager.getAccounts(); ///.getAccountsByType("com.google");
        if (accounts.length > 0 && accounts[0].name != null) {
            ...
            Cursor emailCur = cr.query(ContactsContract.CommonDataKinds.Email.CONTENT_URI, null, where, whatarr, null);
